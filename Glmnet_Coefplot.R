...
download.file(
  ...
  destfile=file.path("D:\\Programs\\R\\R_Sessions", 'manhattan_Test.rds'),
  mode='wb')

# manhattan_Train.rds
download.file(
  'https://query.data.world/s/4tjm263dwjq5knfs5upekzlmzc6oa2',
  destfile=file.path("D:\\Programs\\R\\R_Sessions", 'manhattan_Train.rds'),
  mode='wb')
...
download.file(
  ...
  destfile=file.path("D:\\Programs\\R\\R_Sessions", 'manhattan_Train.csv'),
  mode='w')
...
########################## PRogram Starts Here #########################################
man_data <- readr::read_csv("D:\\Programs\\R\\R_Sessions\\manhattan_Train.csv")

head(man_data)
str(man_data)
x<-c("rsample","coefplot","dygraphs","recipes","xgboost","glmnet")
